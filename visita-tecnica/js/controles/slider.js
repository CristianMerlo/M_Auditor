/* Slider 0-100 + observacion. Default 100. Aporta su valor directo. "No aplica"
   con motivo obligatorio lo excluye. Foto (si el item la admite) aparece <70. */
(function(){
  'use strict';
  ...
  function render(item,d,alCambiar){
    ...
    if(!d.na){
      ...
      ay.textContent='0 = crítico · 100 = sin observaciones'; t.appendChild(ay);
      var obs=document.createElement('textarea'); obs.className='campo'; obs.placeholder='Qué hay que mejorar (opcional)';
      obs.value=d.obs||''; obs.addEventListener('input',function(){d.obs=obs.value;alCambiar(d,{});});
      t.appendChild(obs);
