window.PDF=(function(){
  ...
  function colorSem(sem){var m={verde:'verde',amarillo:'amarillo',rojo:'rojo',naranja:'marca',gris:'sinMedir'};return rgb(m[sem]||'sinMedir');}
  ...
  function construir(v,fotos,firmaJefe,firmaEnc){
    ...
    function encabezado(){ doc.setFillColor(cR[0],cR[1],cR[2]); doc.circle(W/2,14,3.5,'F');
      ...
      doc.text('AUDITORÍA TÉCNICA',W/2,24,{align:'center'});
      doc.setFontSize(7.5); doc.setTextColor(cLabel[0],cLabel[1],cLabel[2]); doc.setFont('helvetica','normal');
      doc.text(fechaCorta(v.iniciada),mx,14);
      doc.text(v.codigoValidacion||v.codigo||'',W-mx,14,{align:'right'});
